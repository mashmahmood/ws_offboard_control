    ...
    def timer_callback(self) -> None:
        ...
        match self.current_state:
            ...
            case "LOITER": 
                ...
                self.get_logger().info(f"Loiter, What Happening: {self.flightCheck}")
                if(not(self.flightCheck)):
                    self.current_state = "IDLE"
                    self.get_logger().info(f"Loiter, Flight Check Failed")
                elif abs(self.vehicle_local_position.z - self.takeoff_height) < 0.1 or (self.nav_state == VehicleStatus.NAVIGATION_STATE_AUTO_LOITER):
                    self.get_logger().info(f"Reached Takeoff Height: {self.vehicle_local_position.z}")
